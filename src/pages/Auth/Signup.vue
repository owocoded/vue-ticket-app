  ...
  if (validateForm()) {
    try {
      ...
      if (success) {
        ...
        authStore.login(email.value, password.value, stayLoggedIn.value)
        router.push('/dashboard')
        if (toastRef.value) {
          toastRef.value.showToast('Account created successfully!', 3000);
        }
      }
    } catch (error: any) {
      errors.signup = error.message || 'Signup failed. Please try again.'
      if (toastRef.value) {
        toastRef.value.showToast(error.message || 'Signup failed. Please try again.', 3000);
      }
    }
  }
}
